...
export default function Calculator() {
  ...
  function symbolClick(symbol) {
    ...
    if (symbols.test(displayText.charAt(displayText.length - 1)) && 
    symbols.test(displayText.charAt(displayText.length - 2))){
      //if last two chars are symbols, replace both
      const newT = displayText.slice(0, displayText.length - 2);
        setNewText(symbol)
        setText(newT + symbol)
        return
    }
    if (/[\+\-\/\*]/.test(displayText.charAt(displayText.length - 1))) {
      //if the last character is an operand, replace it with new symbol
